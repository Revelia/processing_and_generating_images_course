...
- UNET_MSE_FACTOR_010 = UNET_MSE_FACTOR_10, во время запуска экспериментов я случайно поставил для этого эксперимента вес 0.1, заметил только после. Если успею, запущу этот эксперимент корректно тоже, но в целом понятно, что вряд ли результаты с только MSE лоссом будут лучше 
...
Я предполагаю, что заставляя модель учиться исправлять артефакты, разница между реконструкцией и оригиналом будет еще выше.

Если делать такую аугментацию над всем train лоадером, тогда формально мы будем считать loss между изображением с артефактом и его реконструкцией, то есть будем наоборот помогать модели правильно реконструировать аномальный сет, поэтому loss считается именно между оригиналом и реконструкцией по шумным данным
...
| Метрика      | Порог      | TP  | FP  | TN   | FN  | TPR    | TNR    |
|--------------|------------|-----|-----|------|-----|--------|--------|
| Best F1      | 0.8671     | 121 | 564 | 3101 | 8   | 0.9380 | 0.8461 |
| TPR 95       | 0.8831     | 121 | 556 | 3109 | 8   | 0.9380 | 0.8483 |
| TNR 95       | 0.6321     | 129 | 803 | 2862 | 0   | 1.0000 | 0.7809 |
| TNR TPR AVG  | 0.7576     | 127 | 613 | 3052 | 2   | 0.9845 | 0.8327 |

Особо не повлияло

## Итог

В целом, результаты не очень. 

Такое ощущение, что распределение "проливов" и "не проливов" в тестовых данных какое-то другое, потому что для тех, которые на валидации, вроде как метрики получается получить очень высокие, следовательно, по каким то причинам валидация некорректная.

Например: график метрик на валидации для MSE_FACTOR 0.3 и фичами VGG
